...
import { addMaterialRequest, updateMaterialRequest } from "../../API/api";
...
const EditMaterialRequest = () => {
  ...
  const [mrHeaderData, setMrHeadData] = useState([]);
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:5000/api/MR_Header/${currentItemID}`)
      .then((res) => {
        setMrHeadData(res.data[0]);
      })
      .catch((err) => {
        console.error("Error fetching MR Header data:", err);
        setLoading(false);
      });
  }, []);
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:5000/api/mr_Tran/${currentItemID}`)
      .then((res) => {
  ...
  const handleConfirmSelection = (productsWithQty) => {
    setProductList((prevList) => {
      ...
      productsWithQty.forEach((incomingProduct) => {
        ...
        if (existingIndex !== -1) {
          // Update quantity
          updatedList[existingIndex].MR_Qty = incomingProduct.quantity;
        } else {
          // Add new product
          updatedList.push({
            ...incomingProduct,
            Product_UM: incomingProduct.Stock_UM,
            MR_Qty: incomingProduct.quantity,
          });
        }
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      await updateMaterialRequest(currentItemID, payload);
      toast.success("Material Request Updated");
      setMrHeadData([]);
      // navigate("material-request/view");
      ...
      <div>
        ...
        <Box sx={{ backgroundColor: "whitesmoke", minHeight: "91vh" }}>
          ...
          <Box
            ...
            padding={2}
          >
            <Typography variant="h5">Edit Material Request Master</Typography>
          ...
          <Box display="flex" justifyContent="center" marginTop={2}>
            ...
            >
              <TextField
                ...
                name="MR_Date"
                value={
                  mrHeaderData?.MR_Date
                    ? dayjs(mrHeaderData.MR_Date).format("YYYY-MM-DD")
                    : ""
                }
              ...
              <FormControl fullWidth margin="normal">
                <Autocomplete
                  ...
                  value={
                    supplierList.find(
                      (item) =>
                        item.Supplier_Code === mrHeaderData.Supplier_Code
                    ) || null
                  }
              ...
              <TextField
                ...
                name="RequiredDate"
                value={
                  mrHeaderData?.Required_Date
                    ? dayjs(mrHeaderData.Required_Date).format("YYYY-MM-DD")
                    : ""
                }
                onChange={handleInputChange}
                type="date"
              ...
              <Box>
                <FormControl fullWidth margin="normal">
                  <Autocomplete
                    ...
                    value={
                      locationList.find(
                        (item) => item.Location_ID === mrHeaderData.Location_ID
                      ) || null
                    }
                    isOptionEqualToValue={(option, value) =>
                      ...
                      {productList.map((product) => {
                        return (
                          <TableRow key={product.Barcode}>
                            ...
                            <TableCell>{product.Product_UM}</TableCell>
                            ...
                            >
                              {editingBarcode === product.Barcode ? (
                                <TextField
                                  ...
                                  sx={{ width: 80 }}
                                  inputProps={{ step: "0.01" }} // allows input like 2.50
                                />
                              ) : (
                                Number(product.MR_Qty).toFixed(2) // display always with 2 decimal places
                              )}
                            </TableCell>
                          </TableRow>
                        );
