...
const InvoiceList = () => {
    ...
    try {
      ...
      if (invoiceNumber.trim()) {
        formatted = formatted.filter((row) =>
          row.invoice_number?.toLowerCase().includes(invoiceNumber.toLowerCase())
        );
      }
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => (
        <span
          onClick={() => navigate(`/invoice/preview/${params.value}`, { 
    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        if (!code) return "";
        const loc = locationList.find((l) => String(l.Location_ID) === String(code));
